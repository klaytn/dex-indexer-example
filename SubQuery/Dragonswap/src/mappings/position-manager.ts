...
import { initializeFactory } from "./factory";

async function getPosition(
  event: EthereumLog,
  tokenId: BigNumber
): Promise<Position | null> {
  // Trigger initializeFactory here because this handler is called frequently
  await initializeFactory(event.blockNumber);
...
export async function handleDecreaseLiquidity(
  ...
  position.withdrawnToken0 = amount0 + position.withdrawnToken0;
  position.withdrawnToken1 = amount1 + position.withdrawnToken1;
...
export async function handleCollect(
  ...
  const amount0 = convertTokenToDecimal(event.args.amount0, token0.decimals);
  const amount1 = convertTokenToDecimal(event.args.amount1, token1.decimals);
  position.collectedFeesToken0 = amount0 + position.collectedFeesToken0;
  position.collectedFeesToken1 = amount1 + position.collectedFeesToken1;
